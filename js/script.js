...
var allNavLis = document.getElementById("mainNav").getElementsByTagName("li");
var mainHeader = document.getElementById("mainHeader");
var mainNav = document.getElementById("mainNav");
var menuIcon = document.getElementById("menuIcon");
var logo = document.getElementById("logo");
var allNavAs = document.getElementById("mainNav").getElementsByTagName("a");
var iconNum = 1;
var scrollNum = 0;
...
function position() {
    checkPadding();
    if (window.pageYOffset <= 50 && window.innerWidth < 599 || window.pageYOffset <= 50 ||
        window.pageYOffset >= 50 && window.innerWidth > 599 ) {
...
menuIcon.addEventListener("click", function() {
    for (var i = 0; i < allNavLis.length; i++) {
        allNavLis[i].classList.toggle("displayTabs");
    }
    if (iconNum === 1) {
...
function resize() {
    ...
    if (window.innerWidth > 599) {
        for (var i = 0; i < allNavLis.length; i++) {
        allNavLis[i].classList.remove("displayTabs");
        }
        menuIcon.setAttribute("src", "css/images/mobile-menu-icon.png");
        iconNum = 1;
        mainNav.style.position = "absolute";
...
function setPadding(className) {
    for (var i = 0; i < allNavAs.length; i++) {
        allNavAs[i].setAttribute("class", className);
    }
}